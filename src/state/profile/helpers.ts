...
import { OasisswapProfile } from 'config/abi/types/OasisswapProfile'
...
const transformProfileResponse = (
  profileResponse: Awaited<ReturnType<OasisswapProfile['getUserProfile']>>,
): Partial<Profile> => {
...
export const getProfileAvatar = async (address: string) => {
  try {
    const profileCalls = ['hasRegistered', 'getUserProfile'].map((method) => {
      return { address: getOasisswapProfileAddress(), name: method, params: [address] }
    })
    ...
    if (isActive) {
      ...
      if (apiRes) {
        ...
        Cookies.set(
          ...
          { domain: 'oasisswap.finance', secure: true, expires: 30 },
        )
      }
    }
