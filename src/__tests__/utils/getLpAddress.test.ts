...
const OS_BUSD_LP = '0x804678fa97d91B974ec2af3c843270886528a9E6'

describe('getLpAddress', () => {
  it('returns correct LP address, both tokens are strings', () => {
    expect(getLpAddress(OS_AS_STRING, BUSD_AS_STRING)).toBe(OS_BUSD_LP)
  })
  it('returns correct LP address, token1 is string, token 2 is Token', () => {
    expect(getLpAddress(OS_AS_STRING, BUSD_AS_TOKEN)).toBe(OS_BUSD_LP)
  })
  it('returns correct LP address, both tokens are Token', () => {
    expect(getLpAddress(OS_AS_TOKEN, BUSD_AS_TOKEN)).toBe(OS_BUSD_LP)
  })
