...
export const useVaultPoolContract = (vaultKey: VaultKey): OsVault | IfoPool => {
  const { library } = useActiveWeb3React()
  return useMemo(() => {
    return vaultKey === VaultKey.OsVault
      ? getOsVaultContract(library.getSigner())
...
export const useOasisSquadContract = () => {
  const { library } = useActiveWeb3React()
  return useMemo(() => getOasisSquadContract(library.getSigner()), [library])
}
