...
import { useOs, useTokenContract } from './useContract'
...
export const useTotalSupply = () => {
  ...
  const { data } = useSWRContract([osContract, 'totalSupply'], {
...
export const useGetRoseBalance = () => {
  const { account } = useWeb3React()
  const { status, data, mutate } = useSWR([account, 'roseBalance'], async () => {
