...
const LotteryCardContent = () => {
  ...
  const osPriceBusdAsString = usePriceOsBusd().toString()

  const osPrizesText = t('%osPrizeInUsd% in OS prizes this round', { osPrizeInUsd: osPriceBusdAsString })
  const [pretext, prizesThisRound] = osPrizesText.split(osPriceBusdAsString)

  const osPriceBusd = useMemo(() => {
    return new BigNumber(osPriceBusdAsString)
  }, [osPriceBusdAsString])
  ...
  useSlowRefreshEffect(() => {
    ...
    const fetchCurrentLotteryPrize = async () => {
      ...
      const prizeInBusd = osPriceBusd.times(amountCollectedInOs)
  ...
  }, [lotteryId, setCurrentLotteryPrize, osPriceBusd])
