...
import { useOs, useSousChef, useVaultPoolContract } from 'hooks/useContract'
...
export const useVaultApprove = (vaultKey: VaultKey, setLastUpdated: () => void) => {
  ...
  const handleApprove = async () => {
    const tx = await callWithGasPrice(osContract, 'approve', [vaultPoolContract.address, MaxUint256])
    toastSuccess(`${t('Transaction Submitted')}!`, <ToastDescriptionWithTx txHash={tx.hash} />)
    setRequestedApproval(true)
    const receipt = await tx.wait()
    if (receipt.status) {
  ...
  const key = useMemo<UseSWRContractKey>(
    () =>
      account
        ...
        : null,
    [account, osContract, vaultPoolContract.address],
  )
