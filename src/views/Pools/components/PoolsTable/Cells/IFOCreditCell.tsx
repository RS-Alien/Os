...
const IFOCreditCell: React.FC<IFOCreditCellProps> = ({ account }) => {
  ...
  const avgBalanceDollarValue = useBUSDOsAmount(osAsNumberBalance)
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    <>
      <Text>
        {t(
          'Your entry limit in the next IFO sale is determined by your IFO credit. This is calculated by the average OS balance of the principal amount in the IFO pool during the last credit calculation period.',
        )}
      </Text>
      <Text>
        {t(
            ...
            <Flex>
              <Box mr="8px" height="32px">
                <Balance
                  ...
                  value={hasCredit ? osAsNumberBalance : 0}
                />
                {hasCredit ? (
                  <Balance
                    display="inline"
