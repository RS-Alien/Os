...
import { convertSharesToOs, getOsVaultEarnings } from 'views/Pools/helpers'
...
const getAutoEarningsInterestBreakdown = (
  ...
  return getInterestBreakdown({
    principalInUSD: getBalanceNumber(osAsBigNumber.times(stakingTokenPrice)),
    apr: rawApr,
    earningTokenPrice,
...
const AutoEarningsCell: React.FC<AutoEarningsCellProps> = ({ pool, account }) => {
  ...
  const {
    userData: { isLoading: userDataLoading, osAtLastUserAction, userShares, lastUserActionTime },
  ...
  const earnedOsPerHour = hourDiffSinceLastAction ? earningTokenBalance / hourDiffSinceLastAction : 0
    ...
    <>
      ...
      {hourDiffSinceLastAction ? (
        <>
          ...
          <Text bold>{t('OS per hour: %amount%', { amount: earnedOsPerHour.toFixed(2) })}</Text>
