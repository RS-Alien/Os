...
const ContributeModal: React.FC<Props> = ({
  ...
  const label = currency === tokens.os ? t('Max. OS entry') : t('Max. token entry')
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    poolId === PoolIds.poolBasic
      ? t(
          'For the basic sale, Max OS entry is capped by minimum between your average OS balance in the IFO OS pool, or the pool’s hard cap. To increase the max entry, Stake more OS into the IFO OS pool',
        )
      : t(
          'For the unlimited sale, Max OS entry is capped by your average OS balance in the IFO OS pool. To increase the max entry, Stake more OS into the IFO OS pool',
        ),
    {},
  )
        ...
        <Box p="2px">
          ...
          <Text color="textSubtle" fontSize="12px" mb="24px">
            ...
            <Link
              fontSize="12px"
              display="inline"
              href="https://docs.oasisswap.finance/products/ifo-initial-farm-offering"
              external
            >
              {t('Read more')}
            </Link>
