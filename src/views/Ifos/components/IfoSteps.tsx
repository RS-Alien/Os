...
import VaultStakeModal from 'views/Pools/components/OsVaultCard/VaultStakeModal'
...
const Step1 = ({ hasProfile }: { hasProfile: boolean }) => {
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    <Box>
      ...
      <InlineLink
        external
        href="https://medium.com/oasisswap/initial-farm-offering-ifo-3-0-ifo-staking-pool-622d8bd356f1"
      >
  ...
  const creditDollarValue = useBUSDOsAmount(getBalanceNumber(credit))
  ...
  return (
    <CardBody>
      ...
      <Heading as="h4" color="secondary" mb="16px">
        {t('Stake OS in IFO pool')}
      </Heading>
      <Box>
        <Text color="textSubtle" small>
          {t(
            'The maximum amount of OS user can commit to all the sales combined, is equal to the average OS balance in the IFO OS pool prior to the IFO. Stake more OS to increase the maximum OS you can commit to the sale. Missed this IFO? You can keep staking in the IFO OS Pool to join the next IFO sale.',
          )}
        </Text>
      ...
      {hasProfile && (
        <SmallStakePoolCard borderRadius="default" p="16px">
          <FlexGap justifyContent="space-between" alignItems="center" flexWrap="wrap" gap="16px">
            ...
            {isVaultApproved ? (
              <Button onClick={onPresentStake}>{t('Stake')} OS</Button>
            ) : (
