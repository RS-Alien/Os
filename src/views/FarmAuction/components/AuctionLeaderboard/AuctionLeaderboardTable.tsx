...
interface LeaderboardRowProps {
  bidder: Bidder
  osPriceBusd: BigNumber
  isMobile: boolean
}

const LeaderboardRow: React.FC<LeaderboardRowProps> = ({ bidder, osPriceBusd, isMobile }) => {
        ...
        <Flex flexDirection="column" width="100%" justifyContent="flex-end" pr={[null, null, '24px']}>
          ...
          {osPriceBusd.gt(0) ? (
            <Text fontSize="12px" color="textSubtle" textAlign="right">
              ~${getBalanceNumber(amount.times(osPriceBusd)).toLocaleString('en', { maximumFractionDigits: 0 })}
            </Text>
          ) : (
    ...
    <Box>
      <LeaderboardContainer>
        ...
        {bidders.slice(0, visibleBidders).map((bidder) => (
          <LeaderboardRow key={bidder.account} bidder={bidder} osPriceBusd={osPriceBusd} isMobile={isMobile} />
        ))}
      </LeaderboardContainer>
