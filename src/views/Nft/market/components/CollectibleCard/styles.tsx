...
export const CostLabel: React.FC<CostLabelProps> = ({ cost, roseBusdPrice, ...props }) => {
  const priceInUsd = multiplyPriceByAmount(roseBusdPrice, cost)
...
export const LowestPriceMetaRow = ({ lowestPrice, isFetching, roseBusdPrice }: LowestPriceMetaRowProps) => {
  const { t } = useTranslation()
  ...
  return (
    <MetaRow title={t('Lowest price')}>
      {isFetching ? (
        ...
        <CostLabel cost={lowestPrice} roseBusdPrice={roseBusdPrice} />
      )}
    </MetaRow>
  )
}
