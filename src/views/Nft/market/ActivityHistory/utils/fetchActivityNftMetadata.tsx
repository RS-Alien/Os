...
export const fetchActivityNftMetadata = async (activities: Activity[]): Promise<NftToken[]> => {
  const hasPBCollections = activities.some(
    (activity) => activity.nft.collection.id.toLowerCase() === oasisswapBunniesAddress.toLowerCase(),
  )
  let bunniesMetadata
  if (hasPBCollections) {
    bunniesMetadata = await getNftsFromCollectionApi(oasisswapBunniesAddress)
  }

  const pbNfts = bunniesMetadata
    ? activities
        .filter((activity) => activity.nft.collection.id.toLowerCase() === oasisswapBunniesAddress.toLowerCase())
        .map((activity) => {
