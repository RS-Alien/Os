...
import { getOasisswapRabbitsAddress } from 'utils/addressHelpers'
...
export const useGetCollectionDistributionPB = () => {
  ...
  useEffect(() => {
    const fetchTokens = async () => {
      ...
      try {
        apiResponse = await getNftsFromCollectionApi(oasisswapBunniesAddress)
      } catch (error) {
      ...
      const totalCountCall = {
        address: getOasisswapRabbitsAddress(),
        name: 'totalSupply',
      }
      ...
      try {
        const response = await multicallv2(oasisswapRabbitsAbi, bunnyContractCalls)
